...
export interface TechGlyph {
  path: string;
  hex: string;
  /** Defaults to "0 0 24 24" (Simple Icons' own viewBox) when omitted. */
  viewBox?: string;
}

// Simple Icons dropped Oracle's trademarked Java "coffee cup" glyph — the
// one mark everyone actually recognizes as "Java" at a glance — so it's
// hand-authored here from Devicon's MIT-licensed java-plain.svg instead of
// falling back to the much less legible OpenJDK duke mark.
const JAVA_ICON: TechGlyph = {
...
const TECH_ICONS: Record<string, TechGlyph> = {
  ...
  firestore: siFirebase, // Firestore is part of the Firebase suite
  firebase: siFirebase,
};

export function getTechIcon(name: string): TechGlyph | undefined {
